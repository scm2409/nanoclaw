 ...
 * not present in Node. There's no env var to disable it, so we polyfill
 * instead — in-memory, reset on every restart. The room/timeline sync token
 * survives restarts via SqliteStateAdapter (see chat-sdk-bridge.ts) so this
 * doesn't cause message history to be re-fetched; only E2EE session state
 * (device keys, megolm sessions) gets rebuilt from the server key backup
 * via MATRIX_RECOVERY_KEY on each boot.
 */
