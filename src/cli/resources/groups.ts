  ...
  customOperations: {
    ...
    'config update': {
      ...
      handler: async (args) => {
        ...
        if (args['log-subagents'] !== undefined || args.log_subagents !== undefined) {
          const raw = String(args['log-subagents'] ?? args.log_subagents);
          if (!['true', 'false'].includes(raw)) {
            throw new Error('--log-subagents must be one of: true, false');
          }
