...
`core_saveMedia` and `core_deleteMedia` are in the allowlist and are review-gated like page writes, returning the same `status` / `pendingId` / `target` shape. That gating arrived later than the rest of the allowlist, so verify it against the wiki you are actually wiring rather than assuming it: a media write that comes back without a `status` and without a `pendingId` was applied live, and that wiki's plugin still needs updating. The skill tells the agent to report whichever it got; if you write your own persona text around this integration, keep that distinction rather than flattening it.
