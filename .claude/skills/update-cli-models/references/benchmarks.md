...
| Metric | Measures | Fits |
|---|---|---|
| `aa:intelligence_index` | AA's general-capability composite, current index version, per reasoning effort | The escalation slot |
| `aa:coding_index` | Static coding ability, same split | A coding subagent |
| `aa:agentic_index` | Multi-step planning and tool orchestration, same split | The main slot |
| `aa:cost_per_task` | Dollars for one task of the index end to end, at the measured cache hit rate | Every slot — this is the number a price list cannot give you |
...
- **A cheap model is not a good model with a discount.** In a
  ...
  deficit, so cheap mid-tier models drift to the top of every slot. That is what
  `--min-metric` is for: state the floor the slot has to clear before price gets
  a vote.
