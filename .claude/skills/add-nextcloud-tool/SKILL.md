...
- Enabling more apps later (`-e webdav`, `-e contacts`, `-e notes`, `-e tables`) is a re-run of this same command with extended `--args` — it overwrites the named entry — plus a container restart. No rebuild, and no new vault secret: the existing Basic-auth secret already covers every path on that host.
- **`webdav` specifically needs no further care.** Its tools (`nc_webdav_list_directory`, `_read_file`, `_write_file`, `_delete_resource`, `_move_resource`, ...) go through `WebDAVClient(BaseNextcloudClient)`, which never builds its own httpx transport — so the proxy-bypass bug the shim in Phase 3 fixes for Deck doesn't apply to it. Verified end to end (list root, write a file, read it back, delete it) with only the `--args` change above; enabling other apps later is not guaranteed to be this cheap — check whether their client module constructs its own `httpx.AsyncHTTPTransport`/`httpx.Client` before assuming the shim already covers them (see `client/webdav.py` vs. `client/__init__.py`'s `CalendarClient` note in Phase 3 for the pattern to look for).
