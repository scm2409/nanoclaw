...
- **Host UID/GID alignment** — `ARG AGENT_UID=1000` / `ARG AGENT_GID=1000`, renumbering the image's `node` user. `container/build.sh` and `setup/container.ts` pass the host user's real ids. Containers already run as the host uid (`--user` in `src/container-runner.ts`) so mounted host files stay writable from both sides; without a matching `/etc/passwd` entry that uid has no name, and anything that resolves its own identity breaks — OpenSSH aborts with `No user exists for uid N`. Defaults make a plain `docker build .` a no-op. Per-agent-group images are `FROM` the base, so they inherit it.
