...
- **`progress`** — a background task settled (the SDK's task-notification statuses are terminal by definition). Logged only.
- **`background-settled`** — the same settle, with what it was: `summary`, `subagent`, `durationMs`. When it lands after the turn that delegated the work has ended, the poll loop pushes a short note back into the query, because the CLI's own notification is queued for a turn that would otherwise never come. Filtered and batched: a subagent always qualifies, a shell task only once it has run past `SETTLE_WAKE_THRESHOLD_MS` (60 s), and settles arriving within `SETTLE_COALESCE_MS` (10 s) of each other cost one note between them. Mid-turn settles are never queued — the CLI hands those to the model itself.
